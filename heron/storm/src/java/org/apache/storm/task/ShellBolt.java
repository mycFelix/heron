...
public class ShellBolt implements IBolt {
  ...
  public ShellBolt(String... commands) {
    command = commands;
  }
  ...
  public void prepare(Map stormConf, TopologyContext topologyContext,
    ...
    if (stormConf.containsKey(Config.TOPOLOGY_SUBPROCESS_TIMEOUT_SECS)) {
      workerTimeoutMills = 1000 * (Integer) stormConf.get(Config.TOPOLOGY_SUBPROCESS_TIMEOUT_SECS);
    } else {
      workerTimeoutMills = 1000 * (Integer) stormConf.get(Config.SUPERVISOR_WORKER_TIMEOUT_SECS);
    }
    ...
    Number subpid = process.launch(stormConf, context);
    ...
    heartBeatExecutorService.
        scheduleAtFixedRate(new BoltHeartbeatTimerTask(this), 1, 1, TimeUnit.SECONDS);
  }
  ...
  private void handleLog(ShellMsg shellMsg) {
    ...
    switch (logLevel) {
      ...
      case ERROR:
        LOG.error(msg);
        collector.reportError(new ReportedFailedException(msg));
        break;
      default:
        LOG.info(msg);
        break;
  ...
  private void handleMetrics(ShellMsg shellMsg) {
    ...
    //get metric by name
    IMetric iMetric = context.getRegisteredMetricByName(name);
    if (iMetric == null) {
